...
class footballPlayerVC: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.Btn1.addTarget(self, action:#selector(footballPlayerVC.playerInfoAction), for: .touchUpInside)
        cell.viewPlayerDetail(playerNamesArr: playerNameArr, JerseyNumbersArr: jerseyNumberArr)
        return cell
    }
    
    @IBAction func playerInfoAction(_ sender: AnyObject) {
        let playerName = self.playerNameArr[sender.tag] as! String
        let jerseyNumber = self.jerseyNumberArr[sender.tag] as! String
        let playerInfo = playerName + ", jersey number is " + jerseyNumber
        let alertController = UIAlertController(title: "Player details:", message: playerInfo, preferredStyle: .alert)
